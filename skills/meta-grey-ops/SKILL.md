...
| Advantage+ audience | must be explicit `true/false` in `targeting`; **decide per funnel from offer, pixel volume and age floor — `04` §Creative field traps, not a value to copy from a spec** | spec rejected without it (v23+ Graph rule) | — |
