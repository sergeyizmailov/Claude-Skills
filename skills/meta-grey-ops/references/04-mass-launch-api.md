...
- 🔺 `advantage_audience` must be explicit on ad-set CREATE (v23+; defaults to 1 only for default/relaxed targeting, else errors). Update path doesn't require it, any version. v26+: Housing/Employment/Financial ads with relaxable targeting also error without it. When enabled: age_min limited 18–25, age_max forced 65; location/min-age/language/custom-audience exclusions never expanded. `launch.py` requires it in the spec.
  - **Choosing the value** — a per-funnel call, not a default to copy. ON: pixel has real volume on the optimized event and the offer is broad (retrieval stage beats hand-picked interests; DT inputs are advisory anyway). OFF: cold pixel/new account with no learning signal; a hard eligibility floor the model cannot see (geo-licence, KYC, device, language, income proxy); a curated custom-audience whitelist you do not want widened; or an age floor above 25 — ON caps `age_min` at 18–25 and forces `age_max` 65, silently breaking a 30+ funnel. Exclusions (location, min age, language, custom audiences) survive either setting — the widening hits interests only. Grey caveat: ON exposes the creative to a wider audience faster, so a creative that only survives review in a narrow context burns sooner.
