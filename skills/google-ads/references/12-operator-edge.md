...
**The insight most people miss: measure the percentage against Projected ROAS, not the currently-set
target.** Moving a 500% target to 550% does nothing if Projected ROAS is already 520% — that is a ~6%
real gap. Moving to 600% against the same projection creates the ~15% distance needed to force a
response.

Rules: **never change a target more than once per week**; after a change, the correct move is often to
not touch it the following week. Exceptions only for setup errors, runaway spend, or a tracking break.
...
1. **Raise targets sequentially, not preemptively.** Wait for the change to land, observe 1–2 weeks,
   then raise ROAS target ~10% and budget ~20% in the same pass. Repeat until stable. Reacting before
   the behavior shift shows up wastes a cycle.
2. **Switch to Maximize Conversion Value** where a ROAS floor is not essential — it favors the best
   auctions while holding spend steady, at the cost of losing ROAS as a guardrail.
3. **The guardrail triangle**: realistic ROAS target + realistic budget + a bid cap, rather than any
   single lever.
4. **Lolk's contrarian read**: the change may *decrease* competition in the very best auctions, since
   budget-limited spend now distributes more evenly instead of concentrating. A net positive for
   anyone who does not overreact.

**Fred Vallaeys' framing:** *"Let Google fly the plane but keep control of the flight plan."* Define
objectives, supply clean conversion data, run an independent external monitoring loop. Explicit
warning against **knee-jerk control-adding** the moment behavior shifts.
...
Requires an active tROAS and **budget headroom** — a budget-constrained campaign cannot explore. Test
protocol: run 6–8 weeks and watch the bid-strategy report's **traffic diversity** section, not blended
ROAS.
...
A portfolio pools conversion data across every attached campaign, which speeds learning for
low-volume campaigns — but **one underperforming campaign silently drags the shared target's behavior
for every other campaign in the group**, with no per-campaign visibility into which one.
...
winnable auctions with **no warning in the UI**. The only tell is impression-share-lost-to-rank
climbing on a campaign that should be performing.
...
**No published head-to-head test resolves this.** The honest position: it depends on how broken the
pre-change state already was. A portfolio producing garbage has less to lose from a hard reset than a
stable one being fine-tuned.
...
**The New Customer Acquisition trap** [Kirk Williams]: since a 2023 change, NCA tracking was decoupled
from bidding. **The New Customer Lifetime Value column populates in reports even when the bidding
checkbox is off** — it is hypothetical value, not an active signal. This fools people mid-audit into
believing new-customer bidding is live when it is not. The lever is the campaign-level "Adjust your
bidding to help acquire new customers" checkbox, not the account-level goal being configured.
...
The repeated practitioner line: *"Smart Bidding will optimize toward whatever value you feed it, and
it does that quickly and relentlessly."* **The technology is not the constraint. Data quality is.**

**Stale-value decay is the most common failure in this whole area.** Value rules degrade silently —
no alert fires when a value assigned in a hurry two years ago stops reflecting real profit. The fix is
not better tooling; it is a recurring calendar review tied to CRM-confirmed revenue.
...
**The counter-intuitive one — pausing exact match** [Brad Geddes, explicitly the exception not the
rule]: for **high-volume, non-specific terms** ("EHR software"), an exact-match keyword that initially
...
**AI Max makes match-type-level evaluation structurally impossible.** You cannot cleanly attribute
performance to a match type, which is precisely why n-gram-level and asset-level analysis become the
only viable lenses.
...
**Competitor-name bidding as demand capture, not defense.** Run it as a distinct channel with its own
...
**PMax asset groups follow creative strategy, not bidding logic** [Andrew Lolk]: do not ask "how many
asset groups" — ask "can I create meaningfully differentiated creative for this group." Treat PMax as
"Shopping + DSA-style expansion + retargeting", not a full-funnel campaign, and build creative
retargeting-first (product explainers, bestsellers, UGC, trust signals) rather than brand storytelling.
...
**The budget-recommendation math that stops a bad decision.** Before accepting any Google budget
increase, compute the **actual cost per additional conversion promised**, not the headline delta.
Cited example: doubling a daily budget for a projected **+0.75 conversions per week** — a 14× weekly
cost increase for a fractional gain.
...
- **Segment by campaign. Never read account-level blended data** — it masks a competitor dominating
  branded search while barely appearing in category search. The account number is not actionable.
- **Overlap Rate alone is not a signal.** "High overlap just means you and a competitor are targeting
  the same searches. That's expected." It contextualizes; it does not drive action.
...
- **Position Above Rate is more often a bid signal than a quality signal** — check your own bid levels
...
- **Check "limited by budget" before drawing any competitive conclusion** from low impression share.
  A budget constraint masquerading as a competitive loss is the single most common misread.
- **Hard limit to keep in the model:** it cannot reveal competitor profitability, whether they are
  testing or scaling, or their margin structure. **A competitor outranking you might be losing money on
  that exact keyword.**

## Change-history forensics

The real value is the **user-type filter**, not the change log. Segment by "Google Ads recommendation"
(auto-applied) vs a specific logged-in human vs API/script changes, and an ambiguous "something changed
...
current opt-in state lives on a separate settings surface that Change History does not show — this is
a two-surface audit.** Checking one and not the other produces a wrong conclusion.

Cross-reference the change timestamp against the performance timeseries rather than eyeballing "around
when it dropped", or you will misattribute a drop to a change that merely coincided with a market
shift. GAQL query in `10-api-and-automation.md`.
...
**Auto-apply is not all-or-nothing.** You can opt into specific recommendation *types* individually.
Disable entirely for regulated industries needing manual copy review and for accounts with strict CPA
targets. Never auto-apply "Improve Your Responsive Search Ads" or automated-bidding recommendations.
...
YouTube, GDN, Discover) are at **ad-group level, not campaign level** — a common source of
accidentally inherited Display inventory dragging blended performance, because the advertiser
controlled it at the wrong level or did not know the control existed.

Route access, disputes, and credit issues through the **MCC and documented change history** rather
than a single rep relationship. Change-history export is the evidence trail.
...
- **"Interns with terrifying confidence."** They generate well-reasoned-sounding output from
  incomplete or generic knowledge and miss fundamentals. Rooijmans' example: an agent recommending
  changes while missing that a bid limit was already set inside a portfolio strategy, never flagging
  the conflict.
- **The concrete mechanic worth copying:** an LLM intent-classifier flags irrelevant search terms for
  negation immediately, instead of waiting for a fixed spend threshold — his example, waiting for 100
...
- **Infrastructure reality, against the hype:** real agentic management needs explicit SOPs the agent
  ...
  logging, and an inbound-email loop keeping that file current. Not a thin prompt wrapper.
- **Where the moat actually is:** both agreed technical agent setups are replicable within 1–2 years.
  The durable edge is asking better strategic questions, understanding the client's business model,
  and surfacing contradictions in client thinking an AI cannot independently identify.
...
**BigQuery as the loop-closer.** Export Ads data and join against CRM, margin, and inventory data the
UI cannot join natively, then use that analysis to set the conversion values fed back into Smart
Bidding. This is the actual mechanism by which value-based bidding stops being guesswork — it closes
the stale-value-decay problem above.
