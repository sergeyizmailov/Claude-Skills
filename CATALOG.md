...
| [**meta-grey-ops**](skills/meta-grey-ops) | Launches and runs Meta ads through the Marketing API with the `metaops` CLI (workspace → doctor → plan → apply PAUSED → verify → activate, bulk across ad accounts) plus grey-vertical survival: antidetect/proxy setups, agency accounts, token death, cloaking/review-layer filters, verification gates, per-vertical playbooks. |
